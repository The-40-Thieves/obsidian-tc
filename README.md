...
## The interface: 3 tools, ~118 governed capabilities

By default the server advertises just **three meta-tools** instead of a wall of ~118:
...
1. **Broad.** 118 tools covering the meaningful Obsidian operations, including native Bases (`.base`) support with a real expression-DSL evaluator — the broadest open-source Obsidian MCP surface we know of (surveyed 2026-07).
...
|---|---|---|---|---|
| **obsidian-tc** | ~118 (3-tool facade) | FTS5 BM25 · vector (vec0) · GraphRAG RRF | JWT (HS256/JWKS) + per-vault folder ACL + HITL elicit | OTel + Prometheus + CloudEvents |

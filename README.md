...
![Status: Shipped v1.3.2](https://img.shields.io/badge/Status-Shipped_v1.3.2-success)
...
## The interface: 3 tools, ~106 governed capabilities

By default the server advertises just **three meta-tools** instead of a wall of ~106:
...
1. **Broad.** 106 tools covering the meaningful Obsidian operations, including native Bases (`.base`) support with a real expression-DSL evaluator — the broadest open-source Obsidian MCP surface we know of (surveyed 2026-07).
...
|---|---|---|---|---|
| **obsidian-tc** | ~106 (3-tool facade) | FTS5 BM25 + vector + RRF + graph | JWT (HS256/JWKS) + per-vault folder ACL + HITL elicit | OTel + Prometheus + CloudEvents |

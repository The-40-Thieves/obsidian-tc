...
**Scale:** 1,123 tracked code files · 174,981 lines.

TypeScript 161,538 · JavaScript 9,141 · Python 1,640 · SQL 1,560 · Rust 742 · Shell 360.
...
|---|---:|---:|---|
| `tools/` | 85 | 16,853 | domains m1–m8 + admin. The MCP tool surface |
...
| `memory/` | 2 | 539 | entity extraction and materialization for the memory folder |
...
Derived from `git ls-files packages/server/src` over `.ts`/`.sql`, tests excluded — 390 files across 31 subsystems. Top-level files (`cli.ts`, `hash.ts`, …) belong to no subsystem and are not counted here.
...
**400 modules · 1776 dependencies · 118 distinct subsystem pairs · 799 cross-subsystem imports.**

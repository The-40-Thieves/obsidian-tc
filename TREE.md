...
**Scale:** 1,089 tracked code files · 167,972 lines.

TypeScript 155,660 · JavaScript 8,078 · Python 1,640 · SQL 1,492 · Rust 742 · Shell 360.
...
| `experiential/` | 18 | 4,221 | work-memory tier: activation, retrieval log, forget, citations |
| `runtime/` | 14 | 3,221 | **composition root** (WP5) — stores, governance, wiring, transports, shutdown |
| `cli/` | 24 | 3,117 | arg parsing + subcommands |
...
| `migrations/` | 40 | 1,492 | hand-registered SQL. **Two chains** — see below |
| `scheduler/` | 4 | 1,374 | unified background scheduler + durable job queue (THE-517) |
| `db/` | 15 | 1,358 | provisioning, migrate runner, experiential store |
...
Derived from `git ls-files packages/server/src` over `.ts`/`.sql`, tests excluded — 380 files across 31 subsystems. Top-level files (`cli.ts`, `hash.ts`, …) belong to no subsystem and are not counted here.

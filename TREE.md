...
**Scale:** 1,275 tracked code files · 214,676 lines.

TypeScript 198,450 · JavaScript 11,544 · SQL 1,940 · Python 1,640 · Rust 742 · Shell 360.
...
| `cli/` | 37 | 4,621 | arg parsing + subcommands |
...
Derived from `git ls-files packages/server/src` over `.ts`/`.sql`, tests excluded — 448 files across 31 subsystems. Top-level files (`cli.ts`, `hash.ts`, …) belong to no subsystem and are not counted here.
...
| 758 | `packages/server/src/cli/commands/doctor.ts` |
...
| 613 | `packages/server/src/providers/registry.ts` |
...
**451 modules · 2061 dependencies · 129 distinct subsystem pairs · 942 cross-subsystem imports.**

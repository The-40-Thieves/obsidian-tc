...
**Scale:** 1,127 tracked code files · 175,868 lines.

TypeScript 162,425 · JavaScript 9,141 · Python 1,640 · SQL 1,560 · Rust 742 · Shell 360.
...
| `cli/` | 25 | 3,370 | arg parsing + subcommands |
...
Derived from `git ls-files packages/server/src` over `.ts`/`.sql`, tests excluded — 391 files across 31 subsystems. Top-level files (`cli.ts`, `hash.ts`, …) belong to no subsystem and are not counted here.
...
| 798 | `packages/server/src/cli/args.ts` |
...
**401 modules · 1786 dependencies · 118 distinct subsystem pairs · 801 cross-subsystem imports.**

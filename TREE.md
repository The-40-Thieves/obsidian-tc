...
**Scale:** 1,122 tracked code files · 174,431 lines.

TypeScript 161,005 · JavaScript 9,141 · Python 1,640 · SQL 1,543 · Rust 742 · Shell 360.
...
|---|---:|---:|---|
| `tools/` | 83 | 16,493 | domains m1–m8 + admin. The MCP tool surface |
...
| `cli/` | 24 | 3,141 | arg parsing + subcommands |
...
| `memory/` | 2 | 458 | entity extraction and materialization for the memory folder |
...
Derived from `git ls-files packages/server/src` over `.ts`/`.sql`, tests excluded — 388 files across 31 subsystems. Top-level files (`cli.ts`, `hash.ts`, …) belong to no subsystem and are not counted here.
...
| 753 | `packages/server/src/cli/args.ts` |
...
**399 modules · 1761 dependencies · 118 distinct subsystem pairs · 790 cross-subsystem imports.**

...
| THE-1041 | `describe_capability` and `tools/list` advertised input schemas converted in zod's default `io:"output"` mode, diverging from what the server actually validates on 98 of 163 registered capabilities (GH #934, THE-1041, #938). | CHANGELOG.md (1.30.0) | 1 |
...
| THE-1081 | A vault root reached through a symlinked ancestor (e.g. macOS `$TMPDIR` under `/var` -> `/private/var`) made the native addon refuse every read/write in that vault, while the JS fallback accepted it (THE-1081, #946). | CHANGELOG.md (Unreleased) | 1 |

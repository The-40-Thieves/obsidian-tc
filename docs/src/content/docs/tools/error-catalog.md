...
| `elicit_required` | human confirmation required | A human must approve this call. A client with MCP elicitation gets a prompt; otherwise mint one with `obsidian-tc elicit --hash <args_hash> --tool <name>` and resend. Never reuse an old token. |

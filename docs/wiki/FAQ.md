...
**Why do I only see three tools?** That's the default **triad facade**: `find_capability` / `describe_capability` / `call_capability` keep agent context lean while all 159 tools stay reachable (and directly callable by name). Set `toolFacade.mode` to `domain` or `flat` for other shapes. See **[[Tool Reference]]**.

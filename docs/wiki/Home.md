...
> **Status:** Shipped — **v1.18.0**. Published to npm as provenance-signed packages, with a container image at `ghcr.io/the-40-thieves/obsidian-tc:1.18.0`, a one-click `.mcpb` bundle, and standalone binaries. The surface is **157 tools across 31 domains**, advertised by default through a three-tool facade. Licensed **AGPL-3.0-only**.

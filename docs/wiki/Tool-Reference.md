...
| `delete_note` | destructive | `delete:notes` | Delete a note (to the vault's .trash mirror, or permanently). Destructive — requires confirmation. restore_note reads its undo from a snapshot, which is captured only when the server's snapshots.enabled config is on; the default "trusted-local" posture leaves it off, so a deleted note may have no restore_note path back even though it is confirmed destructive. |

...
import { hasEmbeddingExcludedColumn } from "../../search/indexing/note-plan";
...
export function runAudit(
  ...
  // THE-934: `embedding_excluded = 0` — a chunk withheld from embedding by egress.excludePaths is
  // NEVER counted as a null-embedding defect (it has no chunk_embeddings row on purpose). Guarded
  // by hasEmbeddingExcludedColumn so a cache.db from before this migration (or a bare test
  // fixture) keeps the pre-THE-934 unfiltered query — see note-plan.ts's hasBodyShaColumn for the
  // same defensive pattern.
  const excludedGuard = hasEmbeddingExcludedColumn(db) ? "AND c.embedding_excluded = 0" : "";
  const nullEmb = db
    .prepare(

...
// Memory entities, Workspace memory, session bootstrap, plur read-API proxy). Registered
// onto the same shared ToolRegistry assembled in cli.ts, so M5 lights up on both the stdio
// and HTTP edges alongside M0-M4. Capture/memory/workspace are pure-SQLite (+ vault file writes

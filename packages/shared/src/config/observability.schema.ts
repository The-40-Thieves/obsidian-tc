 ...
 * whitespace) or EVERYTHING (a bare double-star). Refused at config load rather than compiled: the
 * first is a typo that silently protects nothing, the second is a whole-feature off switch wearing
 * a path pattern's clothes — an operator who means it should set `plane.enabled: false`.
 */
export function isUnusableEgressExcludePattern(pattern: string): boolean {
  if (pattern.trim().length === 0) return true;
  const trimmed = normalizeEgressExcludePattern(pattern).replace(/\/+$/, "");

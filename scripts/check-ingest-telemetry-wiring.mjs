...
const RECORD_CALL_RE = /\brecordIngestStats\w*\s*\(/;
const SINK_DELEGATE_RE = /\bonIndexVaultComplete\s*\?\.\s*\(/;
...
function matchParen(source, openParenIndex) {
  let depth = 0;
...
function restOfEnclosingBlock(source, fromIndex) {
  let depth = 0;
  let i = fromIndex;
...
function isRealCallSite(source, matchIndex) {
...
function findIndexVaultCallSites(file, source) {
  const sites = [];
  for (const m of source.matchAll(/\bindexVault\s*\(/g)) {
    if (!isRealCallSite(source, m.index)) continue;
    const line = source.slice(0, m.index).split("\n").length;
    const openParen = m.index + m[0].length - 1;
    const callEnd = matchParen(source, openParen);
    if (callEnd === -1) {
...
function consumerTextAfterCall(source, callEnd) {
  const after = source.slice(callEnd + 1);
  const thenMatch = after.match(/^\s*\.then\s*\(/);
  if (thenMatch) {
    const thenOpenParen = callEnd + 1 + thenMatch[0].length - 1;
    const thenClose = matchParen(source, thenOpenParen);
    if (thenClose !== -1) return source.slice(thenOpenParen, thenClose + 1);
  }
  return restOfEnclosingBlock(source, callEnd + 1);
}
 ...
 *  `=> void;` does not). */
function findSinkAssignments(file, source) {
  const sinks = [];
...
for (const file of files) {
  ...
  callSites.push(...findIndexVaultCallSites(file, source));
  sinkAssignments.push(...findSinkAssignments(file, source));
}
...
for (const site of callSites) {
  ...
  const consumerText = consumerTextAfterCall(source, site.callEnd);
